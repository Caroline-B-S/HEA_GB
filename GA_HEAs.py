...
from HEAs import (df, burgers, shear, K, poisson, delta_volume, T0, dEb, stress,
                  parVEC, parPhi, parK, parStress, parPSFE)
...
# class ConstraintPSFE(Constraint):
#     def __init__(self, config, **kwargs):
#         super().__init__()
#         self.config = config

#     def compute(self, population_dict, base_penalty):
#         value = parPSFE(population_dict['population_array'])
#         bad = value >= self.config['max']

#         distance_max = self.config['max'] - value
#         distance = np.zeros(len(value))
